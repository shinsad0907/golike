class ProxyManager {
    ...
    setupProxyEventListeners() {
        // Thêm proxy context menu action
        const contextMenu = document.getElementById('instagram-context-menu');
        
        // Override context menu click để handle proxy action
        ...
        contextMenu.onclick = (e) => {
            ...
            // Call original handler cho các action khác
    ...
    setupProxyModalEvents() {
        ...
        // Auto refresh mapping when input changes
    ...
    loadExistingProxies() {
        // Load proxies from current IG accounts
        ...
        // Remove duplicates
        const uniqueProxies = [...new Set(existingProxies)];
        
        if (uniqueProxies.length > 0) {
            document.getElementById('proxy-list-input').value = uniqueProxies.join('\n');
            console.log('Loaded existing proxies:', uniqueProxies.length);
        }
    }
            ...
            .map(proxy => {
                ...
                if (parts.length >= 2) {
                    return {
                        ...
                        isValid: this.validateProxy(proxy)
                    };
                }
                return {
                    original: proxy,
    ...
    async applyProxies() {
        ...
        if (proxies.length < this.currentIgAccounts.length) {
            alert(`Số lượng proxy (${proxies.length}) phải bằng hoặc lớn hơn số Instagram accounts (${this.currentIgAccounts.length})!`);
            return;
        }

        // Validate all proxies
        const invalidProxies = proxies.filter(p => !p.isValid);
        ...
        // Get button elements and store original states
        const applyBtn = document.getElementById('apply-proxy-btn');
        const originalText = applyBtn?.textContent || 'Áp dụng Proxy';
        
        try {
            // CHECK: Validate eel connection before proceeding
            if (typeof eel === 'undefined') {
                throw new Error('Eel is not defined - backend connection failed');
            }

            // FIXED: Check for the correct function name
            ...
            if (!saveFunction) {
                // Fallback: Try to call any available save function
            ...
            // FIXED: Call eel function with proper error handling
            console.log('Calling backend function with data...');
            console.log('File path:', 'data/manager-golike.json');
            console.log('Data structure:', {
                totalAccounts: this.instagramManager.golikeAccounts.length,
                selectedAccount: this.instagramManager.selectedGolikeAccount.id_account,
                updatedIgAccounts: updatedAccounts.length
            });
            ...
            if (isSuccess) {
                ...
                console.log('Proxies applied successfully:', {
                    ...
                    validProxies: proxies.filter(p => p.isValid).length,
            ...
            // FIXED: Restore button state safely
            if (applyBtn) {
    ...
    async removeAllProxies() {
        ...
        try {
            ...
            // FIXED: Use the same function detection logic as applyProxies
            console.log('Available eel functions:', Object.keys(eel));
            ...
            // Remove proxy from all IG accounts
    ...
    async testSelectedProxies() {
        ...
        // Lấy proxy của các row được chọn
    ...
    async performProxyTest(proxies) {
        ...
        try {
            ...
            for (let i = 0; i < proxies.length; i++) {
                ...
                // Simulate proxy test (thay bằng API thực tế)
                const testResult = await this.testSingleProxy(proxy);
                
                // Update status trong UI
    ...
    async testSingleProxy(proxy) {
        // Simulate proxy test - thay bằng API call thực tế
        return new Promise(resolve => {
            setTimeout(() => {
                // Random test result cho demo
...
// Extend Instagram Manager để tích hợp Proxy Manager
document.addEventListener('DOMContentLoaded', () => {
    // Chờ InstagramManager được khởi tạo
